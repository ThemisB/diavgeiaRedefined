...
//Class definition
class manage_monades_types extends Page
{
   ...
   function manage_monades_typesCreate($sender, $params)
   {
      ...
      if($aclmanager->Role == 'admin')
      {
         ...
         {
            $apAuth->MonadesTypesTable->LimitStart=0;
            $apAuth->MonadesTypesTable->LimitCount=20;
            $apAuth->MonadesTypesTable->Filter =
      ...
      $apAuth->MonadesTypesTable->Active = false;
      $apAuth->MonadesTypesTable->Active = true;
      $apAuth->MonadesTypesTable->refresh();
      if ($apAuth->MonadesTypesTable->RecordCount==0)
      {
   ...
   function manage_monades_typesJSSubmit($sender, $params)
   {
?>
      msg="";
      result=true;
     ...
     document.getElementById('actionSender').value='';
      if ((document.manage_monades_types.field_name) && (document.manage_monades_types.field_name.value.replace(/^\s+|\s+$/g, '')==""))
      {
        result=false;
        msg+="-Το πεδίο \'Όνομα Τύπου Μονάδας:\' είναι υποχρεωτικό.\n";
      }
...
<?php
   ...
   function Label17BeforeShow($sender, $params)
   {
      ...
      global $aclmanager;
            $query = "SELECT * FROM monades where type_id='" . $apAuth->MonadesTypesTable->ID . "' limit 1";
        ...
        if ($apAuth->Query_General2->RecordCount==0)
        {
           ...
           $this->Label17->Link = "manage_monades_types.php?action=delete&edit_id=" . md5($apAuth->MonadesTypesTable->ID);
   ...
   function fdClassesBeforeShow($sender, $params)
   {
      ...
      $query = "SELECT monades_types.name as name from monades_types WHERE monades_types.ID='" . $apAuth->MonadesTypesTable->ID . "'";
      ...
      $this->editLabel->Link = "manage_monades_types.php?action=edit&edit_id=" . md5($apAuth->MonadesTypesTable->ID);
   }
   ...
   function manage_monades_typesBeforeShow($sender, $params)
   {
      global $apAuth;
      $apAuth->initUserData();
      global $aclmanager;
      ...
      */
      ...
      if(is_object($action))
      {
         ...
         {
            $query = "SELECT * FROM monades_types where MD5(ID)='" . mysql_escape_string($_REQUEST['edit_id']) . "'";
            $apAuth->Query_General->SQL = $query;
            $apAuth->Query_General->LimitCount = "-1";
            $apAuth->Query_General->LimitStart = "-1";
            $apAuth->Query_General->Prepare();
            ...
            {
               $query = "SELECT * FROM monades_types where ID='" . $edit_id->asInteger() . "'";
               $apAuth->Query_General->SQL = $query;
               $apAuth->Query_General->LimitCount = "-1";
               $apAuth->Query_General->LimitStart = "-1";
               $apAuth->Query_General->Prepare();
               ...
               $read_type=$apAuth->Query_General->Fields['type'];

               if ($read_servicetype=='ministry') {$this->field_type->ItemIndex=1;};
               if ($read_servicetype=='other') {$this->field_type->ItemIndex=2;};
            ...
            else if($action->asString() == 'delete')
            {
               ...
               global $apAuth;
               $query = "DELETE FROM monades_types WHERE ID=" . $edit_id->asInteger();
...
alert("'.$this->lbMessages->Caption.'");
   ...
   function btnPostClick($sender, $params)
   {
      ...
      try
      {
         ...
         global $apAuth;
         if (mysql_escape_string($_REQUEST['field_type'])=='1') {$type='ministry';}
         if (mysql_escape_string($_REQUEST['field_type'])=='2') {$type='other';}

         $query =
         "
         INSERT INTO `apofaseis`.`monades_types` (
         `ID` ,
         `type` ,
         `name`
         )
...
global $manage_monades_types;

//Creates the form
$manage_monades_types = new manage_monades_types($application);

//Read from resource file
$manage_monades_types->loadResource(__FILE__);

//Shows the form
$manage_monades_types->show();
