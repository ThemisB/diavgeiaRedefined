...
//Class definition
class manage_current_user extends Page
{
...
<?php
   ...
   function manage_current_userCreate($sender, $params)
   {
      global $apAuth;
...
//      $this->EditUsersTable->Refresh();
   ...
   function manage_current_userJSSubmit($sender, $params)
   {
?>
      msg="";
      result=true;
...
<?php
   ...
   function fdClassesBeforeShow($sender, $params)
   {
      ...
      $this->fdClasses->Link = "manage_current_user.php?action=edit&edit_id=" . md5(md5($apAuth->UserTable->ID).md5('stav'));
   ...
   function manage_current_userBeforeShow($sender, $params)
   {
      global $apAuth;
      ...
      if(is_object($action))
      {
         ...
         if(is_object($edit_id))
         {
            ...
            $query = "SELECT * FROM auth where MD5(CONCAT(MD5(ID),MD5('stav')))='" . mysql_escape_string($_REQUEST['edit_id']) . "'";
            $apAuth->Query_General->SQL = $query;
            $apAuth->Query_General->LimitCount = "-1";
            $apAuth->Query_General->LimitStart = "-1";
            $apAuth->Query_General->Prepare();
...
global $manage_current_user;

//Creates the form
$manage_current_user = new manage_current_user($application);

//Read from resource file
$manage_current_user->loadResource(__FILE__);

//Shows the form
$manage_current_user->show();
?>